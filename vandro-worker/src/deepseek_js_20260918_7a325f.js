...
// Vyžaduje secrets: RESEND_API_KEY, MAIL_FROM
// ============================================================
...
Tento e-mail ti prišel, protože ses registroval na <a href="https://naskraj.vandro.cz" style="color:#1B8F52;">naskraj.vandro.cz</a>.
</td></tr>
</table>
</td></tr>
</table>
...
export async function sendEmail(env, { to, subject, html }) {
  if (!env.RESEND_API_KEY) {
    console.warn('RESEND_API_KEY chýba, e-mail sa neposiela:', subject, '→', to);
    return { skipped: true };
  }
  ...
  try {
    ...
    console.error('Resend fetch zlyhal:', err);
...
export async function sendVerificationEmail(env, { to, token, displayName }) {
  ...
  const html = layout('Ověření e-mailové adresy', `
    ...
    <p style="font-size:14px;color:#10201A;line-height:1.6;">pro dokončení registrace na Náš kraj prosím ověř svůj e-mail kliknutím na tlačítko níže. Odkaz je platný <strong>24 hodin</strong>.</p>
    ${button(url, 'Ověřit e-mail')}
    <p style="font-size:12px;color:#64766D;word-break:break-all;">Nebo zkopíruj odkaz: <br>${url}</p>
    <p style="font-size:12.5px;color:#64766D;margin-top:20px;">Pokud jsi se neregistroval, tento e-mail ignoruj.</p>
  `);
...
export async function sendPasswordResetEmail(env, { to, token, displayName }) {
  ...
  const html = layout('Obnovení hesla', `
    <p style="font-size:14px;color:#10201A;line-height:1.6;">Ahoj ${displayName || ''},</p>
    <p style="font-size:14px;color:#10201A;line-height:1.6;">obdrželi jsme žádost o obnovení hesla. Klikni na tlačítko níže a nastav si nové heslo. Odkaz je platný <strong>1 hodinu</strong>.</p>
