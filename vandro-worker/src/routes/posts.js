...
postsRoutes.post('/', async (c) => {
  ...
  if (!rl.ok) return c.json({ error: 'Příliš mnoho příspěvků. Zkus to za hodinu.' }, 429);
  ...
  if (business.user_id !== user.sub && user.role !== 'admin') return c.json({ error: 'Tento podnik nepatrí účtu.' }, 403);
