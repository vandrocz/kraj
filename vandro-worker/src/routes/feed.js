...
async function loadSocialFeed(c, { targetFeed, table, extraFilterCols }) {
  ...
  const sql = `
    ...
    WHERE ${conditions.join(' AND ')} ORDER BY posts.created_at DESC LIMIT 50`;
  ...
  let out = await Promise.all(results.map(async (post) => {
    ...
    return {
      id: post.id, text: post.text_content, html: post.content_html || escapePlain(post.text_content),
      image_url: post.image_url, media: mediaMap[post.id] || (post.image_url ? [post.image_url] : []),
  ...
  if (sort === 'trending' || sort === 'for_you') {
    ...
    out = out.map((p) => {
      ...
      return { ...p, __score: p.likes * 3 + p.comment_count * 5 - ageH * 0.5 };
...
function escapePlain(t) { return String(t || '').replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;'); }
...
feedRoutes.post('/:id/like', async (c) => {
  const user = c.get('user'); const postId = c.req.param('id');
  ...
  if (await c.env.NASKRAJ_LAJKY.get(likeKey)) { const r = await c.env.NASKRAJ_LAJKY.get(`likecount:post:${postId}`); return c.json({ liked: true, likes: r ? parseInt(r, 10) : 0 }); }
  await c.env.NASKRAJ_LAJKY.put(likeKey, '1');
  ...
  if (post.user_id && post.user_id !== user.sub) {
    try { await c.env.DB.prepare(`INSERT INTO notifications (id, user_id, type, actor_id, entity_type, entity_id, text) VALUES (?, ?, 'like', ?, 'post', ?, 'dal(a) like tvému příspěvku')`).bind(newId('notif'), post.user_id, user.sub, postId).run(); } catch {}
  }
  return c.json({ liked: true, likes: n }, 201);
});
...
feedRoutes.post('/:id/comment', async (c) => {
  ...
  if (!mod.clean && mod.severity >= 2) { await flagContent(c.env, { userId: user.sub, reason: mod.reason, severity: mod.severity }); return c.json({ error: 'Zakázaný obsah.' }, 400); }
  const post = await c.env.DB.prepare(`SELECT id, user_id FROM posts WHERE id = ? AND status = 'published'`).bind(postId).first();
  if (!post) return c.json({ error: 'Nenalezeno.' }, 404);
  const id = newId('comment');
  await c.env.DB.prepare('INSERT INTO comments (id, post_id, user_id, comment_text) VALUES (?, ?, ?, ?)').bind(id, postId, user.sub, text).run();
  if (post.user_id && post.user_id !== user.sub) {
    try { await c.env.DB.prepare(`INSERT INTO notifications (id, user_id, type, actor_id, entity_type, entity_id, text) VALUES (?, ?, 'comment', ?, 'post', ?, 'okomentoval(a) tvůj příspěvek')`).bind(newId('notif'), post.user_id, user.sub, postId).run(); } catch {}
  }
...
feedRoutes.post('/:id/report', async (c) => {
  ...
  await c.env.DB.prepare('INSERT INTO reports (id, post_id, reporter_id, reason) VALUES (?, ?, ?, ?)').bind(id, postId, user.sub, body.reason || null).run();
...
feedRoutes.get('/business/:kind/:id', async (c) => {
  ...
  const postsWithMedia = posts.map((p) => ({ ...p, html: p.content_html || escapePlain(p.text_content), media: mediaMap[p.id] || (p.image_url ? [p.image_url] : []) }));
